...
function History() {
    ...
    const handleSaveNotes = () => {
        ...
        // localStorage.setItem('testNotes', JSON.stringify(savedNotes));
    ...
    const NotesModal = () => (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-2xl">
                <h2 className="text-xl font-bold mb-4">Notes for Test #{tests.length - tests.indexOf(selectedTest)}</h2>
                <textarea
                    className="w-full h-64 p-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="Write your notes here..."
                    value={notes[selectedTest?._id] || ""}
                    onChange={(e) => setNotes({...notes, [selectedTest._id]: e.target.value})}
                ></textarea>
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            {showNotesModal && selectedTest && <NotesModal />}
        </div>
    );
}
